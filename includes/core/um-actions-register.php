<?php
	...
	function um_after_new_user_register( $user_id, $args ){
		...
		if ( $pagenow != 'user-new.php' && !array_key_exists( $role, UM()->roles()->get_roles( false, array('admin') ) ) ) {
			$role = um_get_option('default_role');
		}

		UM()->roles()->set_um_user_role( $user_id, $role );
	...
	function um_submit_form_register($args){
		if ( ! isset( UM()->form()->errors ) ) do_action( 'um_user_registration', $args );
